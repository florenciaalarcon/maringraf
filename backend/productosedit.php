...
class cproductos_edit extends cproductos {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("productoslist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->detalle->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->detalle->FldCaption()));
...
<?php if ($productos->titulo->Visible) { // titulo ?>
	<tr id="r_titulo">
		<td><span id="elh_productos_titulo"><?php echo $productos->titulo->FldCaption() ?></span></td>
		<td<?php echo $productos->titulo->CellAttributes() ?>>
<span id="el_productos_titulo" class="control-group">
<input type="text" data-field="x_titulo" name="x_titulo" id="x_titulo" size="30" maxlength="200" placeholder="<?php echo $productos->titulo->PlaceHolder ?>" value="<?php echo $productos->titulo->EditValue ?>"<?php echo $productos->titulo->EditAttributes() ?>>
</span>
<?php echo $productos->titulo->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($productos->imagen->Visible) { // imagen ?>
	<tr id="r_imagen">
		<td><span id="elh_productos_imagen"><?php echo $productos->imagen->FldCaption() ?></span></td>
		<td<?php echo $productos->imagen->CellAttributes() ?>>
<span id="el_productos_imagen" class="control-group">
<span id="fd_x_imagen">
<span class="btn btn-small fileinput-button"<?php if ($productos->imagen->ReadOnly || $productos->imagen->Disabled) echo " style=\"display: none;\""; ?>>
	<span><?php echo $Language->Phrase("ChooseFile") ?></span>
	<input type="file" data-field="x_imagen" name="x_imagen" id="x_imagen">
</span>
<input type="hidden" name="fn_x_imagen" id= "fn_x_imagen" value="<?php echo $productos->imagen->Upload->FileName ?>">
...
<?php echo $productos->imagen->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($productos->detalle->Visible) { // detalle ?>
	<tr id="r_detalle">
		<td><span id="elh_productos_detalle"><?php echo $productos->detalle->FldCaption() ?></span></td>
		<td<?php echo $productos->detalle->CellAttributes() ?>>
<span id="el_productos_detalle" class="control-group">
<textarea data-field="x_detalle" name="x_detalle" id="x_detalle" cols="35" rows="4" placeholder="<?php echo $productos->detalle->PlaceHolder ?>"<?php echo $productos->detalle->EditAttributes() ?>><?php echo $productos->detalle->EditValue ?></textarea>
</span>
<?php echo $productos->detalle->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($productos->precio->Visible) { // precio ?>
	<tr id="r_precio">
		<td><span id="elh_productos_precio"><?php echo $productos->precio->FldCaption() ?></span></td>
		<td<?php echo $productos->precio->CellAttributes() ?>>
<span id="el_productos_precio" class="control-group">
<input type="text" data-field="x_precio" name="x_precio" id="x_precio" size="30" placeholder="<?php echo $productos->precio->PlaceHolder ?>" value="<?php echo $productos->precio->EditValue ?>"<?php echo $productos->precio->EditAttributes() ?>>
</span>
<?php echo $productos->precio->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($productos->id->CurrentValue) ?>">
